...
# Copyright (C) 2017-2026 NV Access Limited, Babbage B.V., Łukasz Golonka, hwf1324, Christopher Proß
...
class TestRectOperators(unittest.TestCase):
	...
	def test_superset(self):
		...
		self.assertFalse(
			RectLTRB(left=2, top=2, right=4, bottom=4).isSuperset(RectLTRB(left=2, top=2, right=6, bottom=6)),
		)
		self.assertTrue(
			RectLTRB(left=2, top=2, right=6, bottom=6).isSuperset(RectLTRB(left=2, top=2, right=6, bottom=6)),
		)

	def test_subset(self):
		self.assertTrue(
			RectLTRB(left=2, top=2, right=4, bottom=4).isSubset(RectLTRB(left=2, top=2, right=6, bottom=6)),
		)
		self.assertFalse(
			RectLTRB(left=2, top=2, right=6, bottom=6).isSubset(RectLTRB(left=2, top=2, right=4, bottom=4)),
		)
		self.assertTrue(
			RectLTRB(left=2, top=2, right=6, bottom=6).isSubset(RectLTRB(left=2, top=2, right=6, bottom=6)),
		)
	...
	def test_equal(self):
		...
		self.assertEqual(
			RectLTRB(left=2, top=2, right=4, bottom=4),
			RectLTWH(left=2, top=2, width=2, height=2),
		)
		self.assertNotEqual(
			RectLTRB(left=2, top=2, right=4, bottom=4),
			RectLTWH(left=2, top=2, width=4, height=4),
		)

	def test_ctypesRECT(self):
		# Intersection
		self.assertEqual(
			RectLTRB(left=2, top=2, right=4, bottom=4).intersection(RECT(left=3, top=3, right=5, bottom=5)),
			RectLTRB(left=3, top=3, right=4, bottom=4),
		)
		# Superset
		self.assertTrue(
			RectLTRB(left=2, top=2, right=6, bottom=6).isSuperset(RECT(left=2, top=2, right=4, bottom=4)),
		)
		# Subset
		self.assertTrue(
			RectLTRB(left=2, top=2, right=4, bottom=4).isSubset(RECT(left=2, top=2, right=6, bottom=6)),
		)
		# in
...
class TestRectUtilities(unittest.TestCase):
	...
	def test_fromFloatCollection(self):
		self.assertEqual(
			RectLTRB(left=10, top=10, right=20, bottom=20),
			RectLTRB.fromFloatCollection(10.0, 10.0, 20.0, 20.0),
		)
		self.assertEqual(
