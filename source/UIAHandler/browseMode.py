...
def UIAHeadingQuicknavIterator(
	...
	if UIAHandler.remote.isSupported():
		if position is None:
			...
			heading = UIAHandler.remote.findFirstHeadingInTextRange(
				searchArea._rangeObj,
				wantedLevel,
				reverse,
			)
...
def UIAControlQuicknavIterator(
	itemType,
	document,
	position,
	UIACondition,
	direction="next",
	itemClass=UIATextRangeQuickNavItem,
):
	...
	def _iterNodesByType(self, nodeType, direction="next", pos=None):
		if nodeType.startswith("heading"):
			...
			condition = createUIAMultiPropertyCondition(
				{
					UIAHandler.UIA_ControlTypePropertyId: [
						UIAHandler.UIA_TableControlTypeId,
						UIAHandler.UIA_DataGridControlTypeId,
					],
				},
			)
			return UIAControlQuicknavIterator(nodeType, self, pos, condition, direction)
			...
			condition = UIAHandler.handler.clientObject.createPropertyCondition(
				UIAHandler.UIA_ControlTypePropertyId,
				UIAHandler.UIA_SeparatorControlTypeId,
			)
			return UIAControlQuicknavIterator(nodeType, self, pos, condition, direction)
			...
			condition = createUIAMultiPropertyCondition(
				{
					...
					UIAHandler.UIA_IsKeyboardFocusablePropertyId: False,
				},
			)
			return UIAControlQuicknavIterator(nodeType, self, pos, condition, direction)
			...
			condition = createUIAMultiPropertyCondition(
				...
				{
					UIAHandler.UIA_ControlTypePropertyId: UIAHandler.UIA_ComboBoxControlTypeId,
					UIAHandler.UIA_IsTextPatternAvailablePropertyId: True,
				},
			)
			return UIAControlQuicknavIterator(nodeType, self, pos, condition, direction)
			...
			condition = createUIAMultiPropertyCondition(
				{
					UIAHandler.UIA_ControlTypePropertyId: UIAHandler.UIA_PaneControlTypeId,
					UIAHandler.UIA_AriaRolePropertyId: ["application", "alertdialog", "dialog"],
				},
			)
			return UIAControlQuicknavIterator(nodeType, self, pos, condition, direction)
