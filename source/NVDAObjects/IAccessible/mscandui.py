...
def reportSelectedCandidate(candidateObject, allowDuplicate=False, newList=False):
	if not eventHandler.isPendingEvents("gainFocus") and (
		allowDuplicate or candidateObject != api.getFocusObject()
	):
		...
		if config.conf["inputComposition"]["autoReportAllCandidates"] and (
			newList or candidateObject.visibleCandidateItemsText != oldCandidateItemsText
		):
			queueHandler.queueFunction(
				queueHandler.eventQueue,
				ui.message,
				candidateObject.visibleCandidateItemsText,
			)
...
class MSCandUI21(IAccessible):
	...
	def event_show(self):
		...
		if role == controlTypes.Role.LIST:
			...
			item = MSCandUI21_candidateMenuItem(
				IAccessibleObject=item.IAccessibleObject,
				IAccessibleChildID=item.IAccessibleChildID,
			)
...
class MSCandUIWindow(IAccessible):
	...
	def event_show(self):
		item = MSCandUIWindow_candidateListItem(
	...
	def refreshCandidateList(self):
		...
		for child in candidateItems:
			if (
				...
				or controlTypes.State.SELECTABLE not in child.states
			):
				continue
...
def findExtraOverlayClasses(obj, clsList):
	...
	if windowClassName == "Microsoft.IME.CandidateWindow.View" and (
		obj.role == controlTypes.Role.BUTTON or obj.role == controlTypes.Role.LISTITEM
	):
