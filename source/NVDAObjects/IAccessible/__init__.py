...
def getNVDAObjectFromEvent(hwnd, objectID, childID):
	...
	obj = IAccessible(
		IAccessibleObject=pacc,
		IAccessibleChildID=accChildID,
		event_windowHandle=hwnd,
		event_objectID=objectID,
		event_childID=childID,
	)
	return obj
...
def normalizeIA2TextFormatField(formatField):
	...
	try:
		...
		fontWeight = None
	if fontWeight is not None and (
		fontWeight.lower() == "bold" or (fontWeight.isdigit() and int(fontWeight) >= 700)
	):
...
class IA2TextTextInfo(textInfos.offsets.OffsetsTextInfo):
	...
	def _getOffsetFromPoint(self, x, y):
		if self.obj.IAccessibleTextObject.nCharacters > 0:
			offset = self.obj.IAccessibleTextObject.OffsetAtPoint(
				x,
				y,
				IA2.IA2_COORDTYPE_SCREEN_RELATIVE,
			)
	...
	def _getTextRange(self, start, end):
		try:
			return self.obj.IAccessibleTextObject.text(start, end) or ""
		except COMError:
			return ""
...
class IAccessible(Window):
	...
	def findOverlayClasses(self, clsList):
		...
		if windowClassName == "Frame Notification Bar" and role == oleacc.ROLE_SYSTEM_CLIENT:
			...
			self.event_objectID == winUser.OBJID_CLIENT
			and self.event_childID == 0
			and windowClassName in ("_WwN", "_WwO")
		):
		...
		if windowClassName.lower().startswith("mscandui") or windowClassName in (
		...
		elif self.event_objectID in (
			None,
			winUser.OBJID_CLIENT,
			MSO_COLLECT_AND_PASTE_OBJECT_ID,
			...
			if (
				windowStyle & winUser.LBS_OWNERDRAWFIXED or windowStyle & winUser.LBS_OWNERDRAWVARIABLE
			) and not windowStyle & winUser.LBS_HASSTRINGS:
			...
			windowStyle = self.windowStyle
			if (
				windowStyle & winUser.CBS_OWNERDRAWFIXED or windowStyle & winUser.CBS_OWNERDRAWVARIABLE
			) and not windowStyle & winUser.CBS_HASSTRINGS:
		...
		if windowClassName == "ConsoleWindowClass" and role == oleacc.ROLE_SYSTEM_CLIENT:
		...
		if (
			...
			if (
				...
				and len(clsList) == 3
				and self.IAccessibleRole == oleacc.ROLE_SYSTEM_CLIENT
	...
	def _isEqual(self, other):
		...
		if self.IAccessibleObject == other.IAccessibleObject:
			return True
		if isinstance(self.IAccessibleObject, IA2.IAccessible2) and isinstance(
			other.IAccessibleObject,
			IA2.IAccessible2,
		):
		...
		if (
			self.event_windowHandle is not None
			and other.event_windowHandle is not None
			and self.event_windowHandle != other.event_windowHandle
		):
			return False
		if (
			self.event_objectID is not None
			and other.event_objectID is not None
			and self.event_objectID != other.event_objectID
		):
			return False
		if (
			self.event_childID is not None
			and other.event_childID is not None
	...
	def _get_IAccessibleIdentity(self):
		if not hasattr(self, "_IAccessibleIdentity"):
			try:
				self._IAccessibleIdentity = IAccessibleHandler.getIAccIdentity(
	...
	re_positionInfoEncodedAccDescription = re.compile(
		r"L(?P<level>\d+)(?:, (?P<indexInGroup>\d+) of (?P<similarItemsInGroup>\d+))?",
	)
	...
	hasEncodedAccDescription = (
		False  #:If true, accDescription contains info such as level, and number of items etc.
	)
		...
		if res:
			...
			if parentObj:
				...
				if (
					...
					and self.IAccessibleRole != oleacc.ROLE_SYSTEM_WINDOW
					and winUser.getAncestor(self.windowHandle, winUser.GA_PARENT) == parentObj.windowHandle
				):
					windowObj = Window(windowHandle=self.windowHandle)
					if (
						windowObj
						and isinstance(windowObj, IAccessible)
						and windowObj.IAccessibleRole == oleacc.ROLE_SYSTEM_WINDOW
	...
	def _get_firstChild(self):
		child = IAccessibleHandler.accNavigate(
			self.IAccessibleObject,
			self.IAccessibleChildID,
		...
		if child and child[0] == self.IAccessibleObject:
			...
			if self.IAccessibleChildID == 0 and child[1] > 0:
				return IAccessible(
					windowHandle=self.windowHandle,
					IAccessibleObject=self.IAccessibleObject,
					IAccessibleChildID=child[1],
					event_windowHandle=self.event_windowHandle,
					event_objectID=self.event_objectID,
					event_childID=child[1],
				)
		elif child:
			obj = IAccessible(IAccessibleObject=child[0], IAccessibleChildID=child[1])
			if (
				obj and winUser.isDescendantWindow(self.windowHandle, obj.windowHandle)
			) or self.windowHandle == winUser.getDesktopWindow():
	...
	def _get_children(self):
		...
		if childCount <= 0:
			return []
		children = []
		for IAccessibleObject, IAccessibleChildID in IAccessibleHandler.accessibleChildren(
			self.IAccessibleObject,
			0,
			childCount,
		):
			...
			if identity and identity.get("objectID", None) == 0 and identity.get("childID", None) == 0:
				...
				if windowHandle:
					...
					APIClass = Window.findBestAPIClass(
						kwargs,
						relation="parent",
					)  # Need a better relation type for this, but parent works ok -- gives the client
					children.append(APIClass(**kwargs))
					continue
			children.append(
				IAccessible(IAccessibleObject=IAccessibleObject, IAccessibleChildID=IAccessibleChildID),
			)
		children = [
			x for x in children if x and winUser.isDescendantWindow(self.windowHandle, x.windowHandle)
		]
		return children
	...
	def getChild(self, index):
		...
		return self.correctAPIForRelation(
			IAccessible(IAccessibleObject=child[0], IAccessibleChildID=child[1]),
		)
			...
			for i in range(nHeaders):
				...
				if self.IA2UniqueID is not None:  # No point checking if we don't have this cell's UID
					try:
						...
						log.debugWarning(
							"could not get IAccessible2::uniqueID to use as headerUniqueID",
							exc_info=True,
						)
						headerUniqueID = None
	...
	def _get_table(self):
		...
		while (
			checkAncestors
			and obj
			and not hasattr(obj, "IAccessibleTable2Object")
			and not hasattr(obj, "IAccessibleTableObject")
		):
			parent = obj.parent = obj.parent
			obj = parent
		if not obj or (
			not hasattr(obj, "IAccessibleTable2Object") and not hasattr(obj, "IAccessibleTableObject")
		):
			return None
		self._table = obj
		return obj
	...
	def _get_positionInfo(self):
		if isinstance(self.IAccessibleObject, IA2.IAccessible2):
			try:
				info = {}
				info["level"], info["similarItemsInGroup"], info["indexInGroup"] = (
					self.IAccessibleObject.groupPosition
				)
				if not info["level"]:
	...
	def _get_isPresentableFocusAncestor(self):
		...
		if (
			IARole == oleacc.ROLE_SYSTEM_CLIENT
			and self.event_objectID == winUser.OBJID_CLIENT
			and self.event_childID == 0
...
class WindowRoot(GenericWindow):
	parentUsesSuperOnWindowRootIAccessible = (
		True  #: on a window root IAccessible, super should be used instead of accParent
	)
		...
		try:
			with windowUtils.threadDpiAwarenessContextOfWindow(window):
				for index in itemIndexes:
					...
					if not user32.GetMenuItemRect(window, menuHandle, index, ctypes.byref(rect)):
						return None
