...
class MSHTML(IAccessible):
	def _get__UIAControl(self):
		if (
			UIAHandler.handler
			and self.role == controlTypes.Role.EDITABLETEXT
			and controlTypes.State.FOCUSED in self.states
		):
			e = UIAHandler.handler.clientObject.getFocusedElementBuildCache(
	...
	def event_caret(self):
		...
		try:
			...
			RuntimeError
		):  # caret events can be fired on the object (focus) when  its not the real MSHTML selection
	...
	def _get_value(self):
		...
		if IARole in (oleacc.ROLE_SYSTEM_PANE, oleacc.ROLE_SYSTEM_TEXT) or (
			IARole == oleacc.ROLE_SYSTEM_COMBOBOX and controlTypes.State.EDITABLE in self.states
		):
			return ""
		else:
			return super().value
	...
	def _get_isFocusable(self):
		...
		if (
			nodeName == "IMG"
			and not self.HTMLNodeHasAncestorIAccessible
			and self.IAccessibleRole == oleacc.ROLE_SYSTEM_GRAPHIC
			and self.IAccessibleStates & oleacc.STATE_SYSTEM_FOCUSABLE
		):
			return True
