...
class MSHTMLTextInfo(VirtualBufferTextInfo):
	...
	def _normalizeControlField(self, attrs: textInfos.ControlField):
		...
		if (
			"HTMLAttrib::onclick" in attrs
			or "HTMLAttrib::onmousedown" in attrs
			or "HTMLAttrib::onmouseup" in attrs
		):
...
class MSHTML(VirtualBuffer):
	...
	def __contains__(self, obj):
		if not obj.windowClassName.startswith("Internet Explorer_"):
			return False
		if (
			not winUser.isDescendantWindow(self.rootDocHandle, obj.windowHandle)
	...
	def _searchableAttribsForNodeType(self, nodeType):
		if nodeType == "link":
			...
			attrs = [
				...
				{
					"IHTMLDOMNode::nodeName": ["H1", "H2", "H3", "H4", "H5", "H6"],
					"HTMLAttrib::aria-level": [nodeType[7:]],
				},
			]
		elif nodeType == "heading":
			attrs = [
				{"IHTMLDOMNode::nodeName": ["H1", "H2", "H3", "H4", "H5", "H6"]},
				{"HTMLAttrib::role": ["heading"]},
			]
		elif nodeType == "list":
			...
			attrs = [
				...
				},
				{
					"IHTMLDOMNode::nodeName": [
						VBufStorage_findMatch_word(node.upper())
						for node, lr in aria.htmlNodeNameToAriaRoles.items()
			...
			attrs = [
				{
					"HTMLAttrib::role": [VBufStorage_findMatch_word(r) for r in ("group", "radiogroup")],
					"name": [VBufStorage_findMatch_notEmpty],
				},
				{
