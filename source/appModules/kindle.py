...
class BookPageViewTreeInterceptor(
	...
	def _iterNodesByType(self, nodeType, direction="next", pos=None):
		if not pos:
			pos = self.makeTextInfo(
				textInfos.POSITION_FIRST if direction == "next" else textInfos.POSITION_LAST,
			)
...
class BookPageViewTextInfo(MozillaCompoundTextInfo):
	def _get_locationText(self):
		...
		text = _("{bookPercentage}%, location {curLocation} of {maxLocation}").format(
			bookPercentage=int((float(curLocation) / float(maxLocation)) * 100),
